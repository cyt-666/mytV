...
impl ApiClient {
    ...
    pub fn refresh_client(&mut self, token: Option<Token>) {
        ...
        headers.insert(
            "trakt-api-key",
            HeaderValue::from_str(&get_config().client_id).unwrap(),
        );

        if let Some(token) = token {
            headers.insert(
                "Authorization",
                HeaderValue::from_str(format!("Bearer {}", token.access_token).as_str()).unwrap(),
            );
